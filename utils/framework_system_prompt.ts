...
const FRAMEWORK_INSTRUCTIONS: Record<string, string> = {
  ...
  "DESC法":
    "DESC法では、入力された対人場面について相手に伝える言葉を、Describe（状況を説明）、Express（今抱えている気持ちとその理由を伝える）、Suggest（代わりの具体的な提案をする）、Consequence（その提案を実行するとどうなるかを伝える）の順に、一人称でそのまま使える自然な日本語で作成してください。Describeでは入力にある出来事や状況を簡潔に説明し、相手の意図を決めつけないでください。Expressでは今の気持ちとそう感じる理由を自分を主語にして述べ、非難を避けてください。Suggestでは困っている状況を改善するための代わりの行動を具体的に提案してください。Consequenceでは提案が実行された場合に見込まれる結果や双方への良い影響を伝えてください。結果を保証したり、従わない場合の脅しを書いたりしないでください。入力にない日時・相手の発言・気持ちの理由などは事実として作らず、不足している場合は仮の表現と分かるように示してください。",
  "ソーシャルスタイル分析":
