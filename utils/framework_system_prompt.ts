...
const FRAMEWORK_INSTRUCTIONS: Record<string, string> = {
  ...
  "SMARTフレームワーク":
    "SMARTフレームワークでは、目標をSpecific（具体的に）、Measurable（測定可能に）、Achievable（達成可能か）、Realistic（関連性）、Time-bound（期限）の順に整理してください。Specificでは何を、なぜ、どのように達成したいのかを明確にし、Measurableでは目標達成度を数値や指標で測定できる形にしてください。Achievableでは現実的に達成可能かを検討し、Realisticでは業務目標との関連性と達成する意味を明確にし、Time-boundではいつまでに達成するのか期限を明示してください。入力情報だけでは具体化できない項目は、推測で補わず確認すべき事項を示してください。",
  "ソーシャルスタイル分析":
