...
const FRAMEWORK_INSTRUCTIONS: Record<string, string> = {
  ...
  "DESC法":
    "DESC法では、入力された対人場面に対して相手へ伝える言葉のたたき台を、Describe（事実を描写）、Express（自分の気持ちや影響を伝える）、Specify（具体的に提案する）、Choose（相手が応じる場合と難しい場合の選択肢を示す）の順に、一人称でそのまま使える自然な日本語で作成してください。Describeでは観察可能な出来事だけを述べ、相手の意図や性格を決めつけないでください。Expressでは非難せず自分を主語にして気持ちや困っている点を伝えてください。Specifyでは相手が判断できる具体的な依頼を丁寧に提案してください。Chooseでは相手に拒否や代案の余地を残し、応じる場合と難しい場合の建設的な対応を示してください。脅しや一方的な通告、譲歩や結果の保証は避けてください。日時・相手の発言など入力にない詳細は事実として作らず、必要に応じて確認を促してください。",
  "ソーシャルスタイル分析":
